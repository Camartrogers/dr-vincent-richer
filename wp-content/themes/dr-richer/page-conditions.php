...
if( $treatments ) {
    ...
    $first_row = $treatments[0];

    // Get the title from the first row
    $first_title = $first_row['treatment_title'];
}
?>
    <div 
    ...
    class='md:col-span-7 md:col-start-4 xl:col-span-6 xl:col-start-5'>
        <section class='mt-14 lg:mt-4'>
            <h2 class="text-[2.5rem] font-manrope leading-tight text-black scroll-reveal"><?= get_field('main_heading'); ?></h2>
            ...
            if( have_rows('treatments') ):
                ...
                ?>
                    <li class='rounded-[10px] bg-white overflow-hidden grid'>
                        ...
                        <?php endif; ?>
                        <div class='px-8 py-8 text-black flex flex-col justify-between'>
                            <div>
                                <p class='font-roboto text-base text-secondary mb-0'><?php echo '0'.$count.'.';?></p>
                                <h3  class='text-2xl font-manrope'><?= get_sub_field('treatment_title');?></h3>
                                <div class='text-lg font-manrope mb-8 [&_a]:underline'><?= get_sub_field('description') ;?></div> 
                            </div>   
                            
                            <button 
                            @click='beforeNow = <?= $count;?>; sliderTitle = "<?= get_sub_field('treatment_title'); ?>"' 
                            class='scroll-to__before-now rounded-full py-2 px-8 mt-auto uppercase text-primary font-roboto border-primary border-2 transition-all duration-150 hover:bg-blackish hover:text-white hover:border-blackish'>See Results</button>
                        </div>
                    </li>
                <?php
                $count++;
